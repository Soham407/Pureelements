...
function AppContent() {
  ...
  const handleUpdateCategories = async (updatedCategories: Category[]) => {
    ...
    <div className={`min-h-screen bg-brand-surface font-sans relative overflow-x-hidden ${!isAdminRoute ? 'pt-[100px] lg:pt-[160px]' : ''}`}>
      ...
      <a
        href="#"
        className="fixed bottom-6 right-6 bg-brand-secondary text-white p-3 rounded-full shadow-lg z-50 hover:bg-brand-secondary/90 transition-colors group"
        aria-label="Chat on WhatsApp"
      >
         <div className="relative">
             <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-brand-secondary"></div>
