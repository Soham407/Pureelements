    ...
    <div className="bg-white p-6 md:p-8 shadow-sm rounded-sm">
      ...
      <div className="space-y-4">
         <label className={`flex items-center gap-4 border p-4 rounded-sm cursor-pointer transition-colors ${!errors.paymentMethod ? 'border-gray-200' : 'border-red-300'}`}>
            <input type="radio" value="UPI" {...register('paymentMethod')} className="accent-brand-primary w-5 h-5" />
            <div>
         ...
         <label className={`flex items-center gap-4 border p-4 rounded-sm cursor-pointer transition-colors ${!errors.paymentMethod ? 'border-gray-200' : 'border-red-300'}`}>
            <input type="radio" value="CARD" {...register('paymentMethod')} className="accent-brand-primary w-5 h-5" />
            <div>
         ...
         <label className={`flex items-center gap-4 border p-4 rounded-sm cursor-pointer transition-colors ${!errors.paymentMethod ? 'border-gray-200' : 'border-red-300'}`}>
            <input type="radio" value="COD" {...register('paymentMethod')} className="accent-brand-primary w-5 h-5" />
            <div>
