...
const HomePage: React.FC<HomePageProps> = ({ 
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4">
          <RevealOnScroll>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                 {gifting.map((item, idx) => (
                    <div key={idx} className="relative aspect-square md:aspect-[4/3] group overflow-hidden cursor-pointer shadow-md" onClick={() => navigate(item.link || '/shop/GIFTING')}>
                        <img src={item.image} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"/>
                        <div className={`absolute inset-0 ${idx % 2 === 0 ? 'bg-black/40' : 'bg-brand-primary/40'} flex flex-col justify-center items-center text-white p-6 text-center`}>
                           <h3 className="font-serif text-2xl md:text-3xl mb-2">{item.title}</h3>
                           <p className="text-[10px] uppercase tracking-[0.2em] opacity-90">{item.subtitle}</p>
                        </div>
                         {idx === 0 && (
         ...
         <RevealOnScroll>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {concerns.map((concern, idx) => (
                <div key={idx} onClick={() => navigate(concern.link || '#')} className="relative aspect-[3/4] cursor-pointer group overflow-hidden bg-gray-200">
            ...
            <RevealOnScroll>
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="w-full lg:w-1/3 relative overflow-hidden h-[300px] lg:h-auto group rounded-sm shadow-md">
                       <img src={bestsellersConfig?.backgroundImage || "/src/assets/bestsellers-bg.png"} className="w-full h-full object-cover brightness-90 group-hover:scale-105 transition-transform duration-1000" alt="Bestsellers" />
                       <div className="absolute inset-0 flex flex-col items-start justify-center p-8 bg-black/10">
                          <h2 className="text-white font-serif text-4xl md:text-5xl drop-shadow-lg text-left leading-tight mb-4 whitespace-pre-line">{bestsellersConfig?.title || "Our\nBestsellers"}</h2>
                          <button onClick={() => navigate('/shop/BESTSELLERS')} className="bg-white text-black px-6 py-2 text-sm uppercase tracking-widest hover:bg-brand-primary hover:text-white transition-colors">
                            {bestsellersConfig?.buttonText || "View All"}
                          </button>
                       </div>
                    </div>
      ...
      {/* Large Promo Banner - Signature Perfumes - Keeping Static for now or generic? */}
      ...
      <section className="py-12 md:py-20 bg-brand-dark text-white">
        <div className="container mx-auto px-4">
           <RevealOnScroll>
              <div className="relative w-full aspect-video md:w-4/5 mx-auto shadow-2xl overflow-hidden group cursor-pointer border border-brand-primary/30" onClick={() => videoSection?.videoUrl && window.open(videoSection.videoUrl, '_blank')}>
                 <img src={videoSection?.thumbnail || "https://picsum.photos/id/452/1280/720"} alt="Video Thumbnail" className="w-full h-full object-cover opacity-75 group-hover:scale-105 transition-transform duration-700" />
                 <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/20 p-4 text-center">
                    <h2 className="font-serif text-2xl md:text-5xl text-brand-surface mb-2 md:mb-4 drop-shadow-xl tracking-wide">{videoSection?.title || "Soulful Scents of Pure Elements"}</h2>
                    <div className="flex items-center gap-4 mb-4 md:mb-8 scale-75 md:scale-100">
                       <span className="h-px w-12 bg-brand-surface"></span>
                       <p className="text-brand-surface tracking-[0.3em] uppercase text-sm font-bold">{videoSection?.subtitle || "Luxury Collection"}</p>
                       <span className="h-px w-12 bg-brand-surface"></span>
                    </div>
            ...
            <RevealOnScroll>
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                  {testimonials.map((t, i) => (
                     <div key={t.id} className="relative aspect-[9/16] bg-gray-800 overflow-hidden shadow-lg group cursor-pointer border border border-gray-200" onClick={() => t.videoUrl && window.open(t.videoUrl, '_blank')}>
                        ...
                        <div className="absolute bottom-3 left-3 right-3 md:bottom-4 md:left-4 md:right-4">
                           <h4 className="text-white text-xs md:text-sm font-bold leading-tight mb-1 drop-shadow-md">{t.name}</h4>
                           <div className="flex items-center gap-1 text-white/80 text-[8px] md:text-[10px]">
                              <Play className="w-2 h-2 md:w-3 md:h-3" fill="currentColor" /> {t.platform}
                           </div>
                        </div>
                     </div>
                  ))}
